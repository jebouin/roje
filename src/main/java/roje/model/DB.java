		...
		if (!f.exists()) {
			try {
				...
				stUserComics.executeUpdate(
						"CREATE TABLE userComics (id INT PRIMARY KEY REFERENCES COMICS(id), mark INT, comment VARCHAR(256), location VARCHAR(100), addprice VARCHAR(100), purchaseDate TIMESTAMP)");
				stComics.close();
				stUserComics.close();
				connect.close();
